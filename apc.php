...
div.info table td.td-0 { border-right:solid rgb(102,102,153) 1px; white-space:class='nwrp'; }
...
EOB;
	...
	if($mem['num_seg']>1 || $mem['num_seg']==1 && count($mem['block_lists'][0])>1)
		$mem_note = "Memory Usage<br><span>(multiple slices indicate fragments)</span>";
	else
			...
			foreach($cache[$list] as $i => $entry) {
				...
				foreach($entry as $k => $value) {
					if (!$AUTHENTICATED) {
						...
						$value=preg_replace('/^.*(\\/|\\\\)/','<em>&lt;hidden&gt;</em>/',$value);
					}
...
EOB;
	...
	$cols=6;
	echo <<<EOB
		<div class=sorting><form><div class='showinline'>Scope:
...
EOB;
  ...
  echo
	...
	foreach($cache[$scope_list[$MYREQUEST['SCOPE']]] as $i => $entry) {
		...
		if (!$AUTHENTICATED) {
			// hide all path entries if not logged in
			$list[$k.$entry[$fieldname]]=preg_replace('/^.*(\\/|\\\\)/','<em>&lt;hidden&gt;</em>/',$entry);
		} else {
	...
	if ($list) {
      ...
      if(!$MYREQUEST['SEARCH'] || preg_match($MYREQUEST['SEARCH'], $entry[$fieldname]) != 0) {
		...
		echo '<tr class=tr-0><td class="center" colspan=',$cols,'><em>No data</em></td></tr>';
	}
	echo <<< EOB
		</tbody></table>
EOB;
	...
	if ($list && $i < count($list)) {
		echo "<a href=\"$MY_SELF&OB=",$MYREQUEST['OB'],"&COUNT=0\"><em>",count($list)-$i,' more available...</em></a>';
	}
...
EOB;
	...
	if ($list) {
		...
		echo '<tr class=tr-0><td class="center" colspan=6><em>No data</em></td></tr>';
	}
	echo <<< EOB
		</tbody></table>
EOB;
...
EOB;
	...
	if (!$rss) {
		...
		while (list(,$v) = each($match[2])) {
			...
			echo "<strong><a href=\"http://pecl.php.net/package/APC/$ver\">".htmlspecialchars($v)."</a></strong><br><blockquote>";
