...
{
	//Fatal error
	//print ('<B>FPDF error: </B>'.$msg);
	//print "\nMe stubborn, try to continue.\n";
	qlog("FPDF/Parse PDF for searchindex error - could not read PDF file. Ignoring it.");
}
	...
	if(!isset($this->fonts[$fontkey]))
	{
		...
		if(isset($this->CoreFonts[$fontkey]))
		{
			if(!isset($fpdf_charwidths[$fontkey]))
			{
				...
				if(defined('FPDF_FONTPATH')) $file=FPDF_FONTPATH.$file;
				include($file);
				if(!isset($fpdf_charwidths[$fontkey])) $this->Error('Could not include font metric file:' . $file);
			}
			$i=count($this->fonts)+1;
