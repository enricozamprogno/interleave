...
function tekenGrafiek($arrX, $arrY, $aankoopArr, $AreDates, $desc, $imgWidth, $imgHeight){
	...
	$ySpan = $maxY - $minY;
	if (!$in_line) {
		qlog(INFO, "SENDING IMAGE PNG HEADER");
...
//	$ShadowColor = ImageColorAllocate ($im, 0, 0, 0);		// Kleur van grafiek zelf
	if (sizeof($arrY) < 2){
		...
		ImageStringup ($im, 2, $imgWidth - imagefontheight(1) - 6, $imgHeight - $onderrand, $GLOBALS['CRM_VERSION'], $ShadowColor);
	...
	if ($filename_to_disk_to_use<>"") {
		qlog(INFO, "Writing to $filename_to_disk_to_use");
