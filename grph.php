...
if (in_array("gd",$a)) {
		...
		print "<span class='noway'>GD Not installed, unable to generate images.</span>";
		print "</div></body></html>";
		exit;
	}
if ($_REQUEST['hits']) {
		require_once($GLOBALS['PATHTOINTERLEAVE'] . "fgrph.php");
		$sql= "SELECT timestamp_last_change FROM " . $GLOBALS['TBL_PREFIX'] . "journal";
		if ($debug) { print "\nSQL: $sql\n"; }
		$result= mcq($sql,$db);
		$x = 0;
		while ($resarr=mysql_fetch_array($result)){
				$x++;
				$table_array[$x] = substr($resarr['timestamp_last_change'],0,8);
		}
		$desc = "Activity (" . $title . " Interleave " . getenv("SERVER_NAME") . ")";
} else {
		ShowHeaders();
		print "<fieldset><legend>&nbsp;<img src='images/crmlogosmall.gif' alt=''>&nbsp;&nbsp;Statistics</legend><img src='grph.php?hits=1' alt=''><br><br>";
		print "Activity stats are based on the journal file Interleave keeps. An activity<br>";
		...
		print "activity of this Interleave installation.<br><br>It could take a while to load as Interleave generates the image on the fly.";
		print "</fieldset></html>";
