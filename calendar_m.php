...
$pred = $_REQUEST['pred'];
	...
	//function prCalendar($fromyear,$frommonth,$fromday,$href,$username,$session) {
		...
		print "<table width='100%'><tr><td><a href='calendar.php?strmonth=" . $prevmonth . "&amp;stryear=" . $prevyear . "'>prev</a></td><td align='right'><a href='calendar_m.php?strmonth=" . $nextmonth . "&amp;stryear=" . $nextyear . "&amp;pred=" . $nextpred . "'>next</a></td></tr></table>";
	prCalendar($stryear,$strmonth,"1","bla","","");
		print "<table width='100%'><tr><td><a href='calendar_m.php?strmonth=" . $prevmonth . "&amp;stryear=" . $prevyear . "'>prev</a></td><td align='right'><a href='calendar_m.php?strmonth=" . $nextmonth . "&amp;stryear=" . $nextyear . "&amp;pred=" . $nextpred . "'>next</a></td></tr></table>";
	?>
	<script type='text/javascript'>
			<!--
			function AdjustDate(date) {

				if ('<?php echo $DateFormat;?>'=='mm-dd-yyyy') {
			...
			{
					window.opener.document.forms['EditEntity'].elements['content'].focus();
					...
					window.opener.document.forms['EditEntity'].elements['content'].focus();
					window.opener.document.forms['EditEntity'].elements['displayDate'].blur();
					window.opener.document.forms['EditEntity'].elements['duedate'].blur();
					window.opener.document.forms['EditEntity'].elements['content'].focus();
					window.close();
					window.opener.document.forms['EditEntity'].elements['content'].focus();
			}
...
function prMonth($year,$month,$href,$fd,$cd){
	...
	if ($monthcounter<($MonthsToShow+1)) {
		...
		echo "<tr align=right><td class='smallsort'>Week</td>\n<td class='smallsort'><strong>Mo</strong></td><td class='smallsort'><strong>Tu</strong></td><td class='smallsort'><strong>We</strong></td><td class='smallsort'><strong>Th</strong></td><td class='smallsort'><strong>Fr</strong></td><td class='smallsort'><strong>Sa</strong></td><td class='smallsort'><strong>Su</strong></td>";
		$wd=date("w",$first);
		if ($wd==0){
			$wd=7;
		}
		...
		for ($k=0;$k<6;$k++){
			...
			if ($cur<=0){
				...
				echo "<tr align=right>\n<td class='smallsort' align='center'><strong><a href='calendar_m.php?weekdetail=$timestamp&amp;year=$year' alt='Go to weekview'>$weeknum</a></strong>\n";
			}
			...
			for ($i=0;$i<7;$i++ ){
				...
				elseif (1==1){
					...
					$maandnaam= (date("F",$timestamp));
						...
						if ($MainPageCalendar) {
							//$sql = "SELECT * FROM " . $GLOBALS['TBL_PREFIX'] . "entity WHERE duedate='" . mres($cur1 . "-" . $month . "-" . $year) . "' AND deleted<>'y'";
							...
							$ins = "<span style='color: #000000;'>";
							  while ($today= mysql_fetch_array($result)) {
									if ($today[category]<>"" && (CheckEntityAccess($row['eID']) == "ok" || CheckEntityAccess($row['eID']) == "readonly")) {
										...
										$html.= $lang[category] . ": " . str_replace("\"","",$today[category]) . "</a><br>";
										$html.= "-------------------------------<br>";
							  ...
							  print "<td><a $ins2 href='summary.php?owner=all&amp;assignee=all&amp;" . $GLOBALS['TBL_PREFIX'] . "customer=all&amp;duedate=$cur-$month-$year&amp;csv=onscreenfull' class='smallsort' >$ins$cur</span></a></td>\n";
							  unset($html);
							  unset($ins);
							  unset($ins2);
						} else {
							...
							print "<td><a href=\"javascript:applydate('" . $cur . "-" . $month . "-" . $year . "')\">$cur </a></td>\n";
						}
					...
					if ($bdarray[1]==$month && $bdarray[2]==$cur){
						...
						if ($MainPageCalendar) {
							...
							print "<td class='smallsort'><a $ins2 href='summary.php?owner=all&amp;assignee=all&amp;" . $GLOBALS['TBL_PREFIX'] . "customer=all&amp;duedate=$cur-$month-$year&amp;csv=onscreenfull' class='smallsort' >$ins$cur</span></a></td>\n";
							unset($html);
							unset($ins);
							unset($ins2);
						} else {
		...
		print "</table>\n\n";
			?>
			<form id='bogus-focus' method='get' action=''><div class='showinline'>
...
function ShowWeek($weeknum,$year) {
	...
	print "<fieldset><legend>&nbsp;<img src='images/crmlogosmall.gif' alt=''>&nbsp;&nbsp;Calendar&nbsp;</legend><table>";
	print "<tr><td><a href='calendar_m.php?weekdetail=" . $prevweek . "&amp;year=" . $year. "'>&lt;&lt;</a>&nbsp;&nbsp;<a href='calendar_m.php?weekdetail=" . $nextweek . "&amp;year=" . $year. "'>&gt;&gt;</a></td></tr>";
	print "<tr><td>&nbsp;</td><td>";
	...
	for ($i=0;$i<$NUMDAYS;$i++) {
		...
		$today = date('l,d M Y');
		$basictoday = date('Ymd',$tmp);
		$tmp += 86400;
		$sql = "SELECT * FROM " . $GLOBALS['TBL_PREFIX'] . "entity WHERE sqldate='" . mres($basictoday) . "'";
		$result = mcq($sql,$db);
		while ($row = mysql_fetch_array($result)) {
			$b .= "&nbsp;<img src='images/info.gif' alt=''>&nbsp;" . $lang['duedate'] . " " . strtolower($lang['entity']) . " " . $row['eid'] . "<br>";
		}
	...
	for ($i=$GLOBALS['CAL_MINHOUR'];$i<$GLOBALS['CAL_MAXHOUR']+1;$i+=.5) {
		...
		if (!$s30) {
			if (substr($txt,0,2) == date('G')) {
				print "<tr><td><strong><span class='noway'>"  . $txt . "</span></strong></td>";
			} else {
				print "<tr><td><strong>"  . $txt . "</strong></td>";
			}
		} else {
			unset($s30);
		}
		...
		for ($c=0;$c<$NUMDAYS;$c++) {
			...
			$sql = "SELECT * FROM " . $GLOBALS['TBL_PREFIX'] . "entity WHERE LEFT(duetime,2)='" . mres(substr($val,0,2)) . "' AND duedate='" . mres($dd) . "'";
			...
			if (!in_array($sql,$sqla)) {
				...
				while ($row = mysql_fetch_array($result)) {
					if ($p<6) {
						$ins .= "<a class='arrow' href='edit.php?e=" . $row['eid'] ."'>" . $row['eid'] . ": " . $row['category'] ."</a><br>";
						$prt=1;
						$p++;
					}
				...
				if ($p==6) {
					$ins .= "<a href='summary.php?owner=all&amp;assignee=all&amp;CRMcustomer=all&amp;duedate=$dd&amp;csv=onscreenfull' class='smallsort'>&lt;more&gt;</a>";
				}
				if (!$prt==1) {
					if ($ovwr) {
							print "<td>";
							print "<table border='1' cellpadding='2' cellspacing='2' style='width: 95%; height: 95%;'><tr><td>&nbsp;";
